...
class WishedGitForm(forms.Form):
    wished_gift = forms.CharField(label="", max_length=200, widget=forms.Textarea(attrs={"rows":"5"}))
...
def draw_name(request, uuid):
    ...
    draw_name_as_receiver = draw.drawname_set.get(to_participant__pk=draw_name.from_participant.pk)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            draw_name_as_receiver.wished_gift = form.cleaned_data.get("wished_gift", None)
            draw_name_as_receiver.save()
            saved = True
    context = {"draw_name": draw_name, "uuid": uuid, "draw_name_as_receiver": draw_name_as_receiver, "form":form, "saved": saved}
